...
function addToCollection(details) {
  collection.push(details);
  updateCollectionUI();
}

function removeFromCollection(imdbID) {
  const index = collection.findIndex((item) => item.imdbID === imdbID);
  if (index !== -1) {
...
function updateCollectionUI() {
  ...
  collection.forEach((item) => {
    ...
    collectionItem.innerHTML = `
            ...
            <li class = "rated">Ratings: ${item.Rated}</li>
        ...
        <p class = "awards"><b><i class = "fas fa-award"></i></b> ${
          item.Awards
                    ...
                    <button class="remove-button" onclick="removeFromCollection('${
                      item.imdbID
                    }')">Remove</button>
                </div>
            `;
...
function displayMovieDetails(details) {
  ...
  addButton.onclick = () => addToCollection(details);
  resultGrid.appendChild(addButton);
}
